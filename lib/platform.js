            ...
            val.forEach((x) => {
              ...
              entries.forEach((subEntry) => {
                ...
                switch (k) {
                  ...
                  case 'extraTwentyOne':
                  case 'extraTwentyTwo':
                  case 'extraTwentyThree':
                  case 'extraTwentyFour':
                  case 'extraTwentyFive':
                  case 'extraTwentySix':
                  case 'extraTwentySeven':
                  case 'extraTwentyEight':
                  case 'extraTwentyNine':
                  case 'extraThirty':
                  case 'extraThirtyOne':
                  case 'extraThirtyTwo':
                  case 'extraThirtyThree':
                  case 'extraThirtyFour':
                  case 'extraThirtyFive':
                  case 'extraThirtySix':
                  case 'extraThirtySeven':
                  case 'extraThirtyEight':
                  case 'extraThirtyNine':
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          this.log(`[AWS] [splitData] ${this.accountTopic}:::${accountToken}:::${this.config.username}:::${this.accountId}:::${this.iotEndpoint}:::${this.iotPass}:::${accountTokenTTR}`)
        ...
        if (this.config.awsDisable) {
          ...
          this.log(`[AWS] ${this.accountTopic}:::${accountToken}:::${this.config.username}:::${this.accountId}:::${this.iotEndpoint}:::${this.iotPass}:::${accountTokenTTR}`)
        }
      } catch (err) {
  ...
  async sendDeviceUpdate(accessory, params) {
    ...
    if (accessory.context.useAWSControl && data.awsParams) {
      try {
        ...
        this.log('[%s] sent AWS update. %s', accessory.displayName, JSON.stringify(data.awsParams))
        return true
      } catch (err) {
