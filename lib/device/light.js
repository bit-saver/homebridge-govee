...
export default class {
  ...
  async internalStateUpdateBT(value) {
    try {
      const newValue = value ? 'on' : 'off'
  ...
  async internalSceneUpdate(charName, awsCode, bleCode, value, isService = false) {
    try {
      ...
      const fnDelay = (fn, delay) => new Promise((resolve) => {
        setTimeout(() => {
          fn().then(resolve);
        }, delay);
      })
      ...
      // Send the request to the platform sender function
      const ptUrl = awsCode.startsWith('ptUrl')
      const sendUpdate = () => this.platform.sendDeviceUpdate(this.accessory, {
        cmd: ptUrl ? 'ptUrl' : 'rgbScene',
        value: [awsCode.replaceAll('ptUrl.', ''), bleCode],
      })
      if (ptUrl) {
        // const darkRainbowScene = 'owABAlgHAkAIKf+ZAP/4JEX//5w\u003D,o/9Gzk7/XPz/Tf9JU9D//wAAAEI\u003D,MwUK7QAAAAAAAAAAAAAAAAAAANE\u003D'
        // await this.platform.sendDeviceUpdate(this.accessory, {
        //   cmd: 'rgbScene',
        //   value: [darkRainbowScene, null],
        // })
        await this.internalStateUpdateBT(0);
        await fnDelay(() => this.internalStateUpdateBT(1), 1000)
        await fnDelay(sendUpdate, 1000)
      } else {
        await sendUpdate()
      }
