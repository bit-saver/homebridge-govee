...
export default class {
  constructor(platform, iotFile) {
    ...
    this.device.on('close', () => {
      platform.log('[AWS] %s.', platformLang.awsEventClose);
      this.connected = false;
    });

    // A listener event for if the connection reconnects
    this.device.on('reconnect', () => {
      platform.log('[AWS] %s.', platformLang.awsEventReconnect);
      this.connected = true;
    });

    // A listener event for if the connection goes offline
    this.device.on('offline', () => {
      platform.log('[AWS] %s.', platformLang.awsEventOffline);
      this.connected = false;
    });

    // A listener event for if the connection creates an error
    this.device.on('error', (error) => {
      platform.log('[AWS] %s [%s].', platformLang.awsEventError, parseError(error));
      this.connected = false;
    });
    ...
    this.device.on('message', (topic, payload) => {
      ...
      try {
        ...
        platform.log('[AWS] %s [%s].', platformLang.invalidJson, payloadString);
        return;
      }
      ...
      if (payload[0]?.sku === 'H61F6') {
        platform.log('[AWS] %s [%s].', platformLang.awsEventMessage, payloadString);
      }
    ...
    this.device.on('connect', () => {
      platform.log('[AWS] %s.', platformLang.awsEventConnect);
      this.connected = true;
    });
  }
  ...
  async requestUpdate(accessory) {
    ...
    accessory.log(`[AWS] ${platformLang.sendingUpdate} ${JSON.stringify(payload)}`);
  ...
  async updateDevice(accessory, params) {
    // Check if we are connected before attempting an update
    if (!this.connected) {
      this.platform.log(`[AWS] No AWS connection on updateDevice: ${this.device} ${platform.iotEndpoint}`);
      throw new Error(platformLang.notAWSConn);
    }
